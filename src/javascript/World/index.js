...
import MarketsSection from './Sections/MarketsSection.js'
...
export default class World
{
    ...
    setReveal()
    {
        ...
        this.reveal.go = () =>
        {
            ...
            // Mashina boshlanish pozitsiyasi
            this.physics.car.chassis.body.sleep()
            this.physics.car.chassis.body.position.set(-28, 0, 2)
            window.setTimeout(() => { this.physics.car.chassis.body.wakeUp() }, 300)
        ...
        this.time.on('tick', () =>
        {
            ...
            {
                for(const key in this.materials.shades.items)
                {
                    this.materials.shades.items[key].uniforms.uRevealProgress.value = this.reveal.matcapsProgress
                }
            ...
            {
                for(const mesh of this.objects.floorShadows)
                {
                    mesh.material.uniforms.uAlpha.value = this.reveal.floorShadowsProgress
                }
    ...
    setZones()
    {
        this.zones = new Zones({ time: this.time, physics: this.physics, debug: this.debugFolder })
    ...
    setTiles()
    {
        this.tiles = new Tiles({ resources: this.resources, objects: this.objects, debug: this.debug })
    }

    setWalls()
    {
        this.walls = new Walls({ resources: this.resources, objects: this.objects })
    }
    ...
    setCar()
    {
        ...
        window.carInstance = this.car
    }

    setMarkets()
    {
        this.markets = new MarketsSection({
            config:    this.config,
            time:      this.time,
            resources: this.resources,
            camera:    this.camera,
        ...
        this.container.add(this.markets.container)
        ...
        this.container.add(this.gps.container)

        // Parkovka tormozi
        window.addEventListener('openShop',  () => this._parkingBrake(true))
        window.addEventListener('closeShop', () => this._parkingBrake(false))
    }

    _parkingBrake(on)
    {
        if(!this.physics?.car) return

        this._brakeActive = on

        if(on)
        {
            this.physics.car.brake()
            this.physics.car.chassis.body.velocity.set(0, 0, 0)
            this.physics.car.chassis.body.angularVelocity.set(0, 0, 0)

            if(!this._brakeTickRegistered)
            {
                this._brakeTickRegistered = true
                this.time.on('tick', () =>
                {
