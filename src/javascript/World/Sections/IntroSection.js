...
export default class IntroSection
{
    ...
    setTitles()
    {
        ...
        context.fillText('ORIPOV OZODBEK', 512, 128)

        const texture = new THREE.CanvasTexture(canvas)
        texture.magFilter = THREE.LinearFilter
        texture.minFilter = THREE.LinearFilter

        const geometry = new THREE.BoxGeometry(10, 2.5, 0.4)
        const material = new THREE.MeshBasicMaterial({ map: texture, transparent: true, color: 0xffffff })
        
        const mesh = new THREE.Mesh(geometry, material)
        mesh.position.set(2, 0, 0)
        
        // Add as a physical object
        this.objects.add({
            base: mesh,
            collision: mesh,
            offset: new THREE.Vector3(2, 0, 0),
            rotation: new THREE.Euler(0, 0, 0),
            shadow: { sizeX: 10, sizeY: 2.5, offsetZ: -0.6, alpha: 0.4 },
            mass: 5,
            soundName: 'brick'
        })
    }
